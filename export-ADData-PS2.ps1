<#
 ...
 .DESCRIPTION
  ...
  Version: 0.8.4 (PowerShell-2.0 compatible edition)
...
process {
    ...
    if ($Computer) {
        ...
        $computers = Get-ADComputer -Filter * -Properties $computerExtraProps -SearchBase "$DNPath" | 
          Where-Object {
            if ($ExcludeSystemObject) {
                $isSys = $false
                if ($_.isCriticalSystemObject -eq $true -or $_.isCriticalSystemObject -eq "TRUE") { $isSys = $true }
                if ($_.sAMAccountName -and ($excludedComputers -contains $_.sAMAccountName)) { $isSys = $true }
                if ($isSys) {
    ...
    $users = Get-ADUser -Filter * -Properties $userExtraProps -SearchBase "$DNPath" | 
      Where-Object {
        if ($ExcludeSystemObject) {
            $isSys = $false
            if ($_.isCriticalSystemObject -eq $true -or $_.isCriticalSystemObject -eq "TRUE") { $isSys = $true }
            if ($_.sAMAccountName -and ($_.sAMAccountName -match '\$$')) { $isSys = $true }
            if ($_.sAMAccountName -and ($excludedUsers -contains $_.sAMAccountName)) { $isSys = $true }
            if ($isSys) {
    ...
    $groups = Get-ADGroup -Filter * -Properties $groupExtraProps -SearchBase "$DNPath" | 
      Where-Object {
        if ($ExcludeSystemObject) {
            $isSys = $false
            if ($_.isCriticalSystemObject -eq $true -or $_.isCriticalSystemObject -eq "TRUE") { $isSys = $true }
            if ($_.sAMAccountName -and ($excludedGroups -contains $_.sAMAccountName)) { $isSys = $true }
            if ($isSys) {
